...
from noip_renewer.error import LoginError
...
def _is_logged_in(driver: webdriver.Firefox) -> bool:
    return len(driver.find_elements(By.ID, "user-email-container")) > 0


def _account_container_appeared(driver: webdriver.Firefox):
...
class NoipRenewer:
    ...
    def login(self):
        ...
        WebDriverWait(self._driver, NoipRenewer.RENEWER_TIMEOUT).until(_account_container_appeared)
    ...
    def logout(self):
        ...
        # Click on top right account container.
        account_container = self._driver.find_element(By.ID, "user-email-container")
        account_container.click()

        # Click the logout button
        logout_icon = account_container.find_element(By.XPATH, "//a/i[@class='fa fa-sign-out']")
        logout_icon.click()
    ...
    def run(self) -> int:
        ...
        WebDriverWait(self._driver, NoipRenewer.RENEWER_TIMEOUT).until(
            EC.presence_of_element_located((By.XPATH, "//thead/tr/th/span[text()='Hostname']"))
        )
...
def make_noip_renewer(driver, credentials: Creds) -> Generator[NoipRenewer, None, None]:
    """
    Create a NoipConfirmer context that logs out then closes the webdriver when it goes out of scope
    """
    try:
